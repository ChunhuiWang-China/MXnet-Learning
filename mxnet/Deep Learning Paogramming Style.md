...
内存分配优化没有被约束在小计算图中。你在大计算图中也可以使用它。然而对于像时CXXNet核Caffe这样的库优化大计算时没有必要的，因为你不能发现他们的编译步骤。然而，在这些库中的编译步骤，通过运行一个接一个的计算将层翻译成一个被修复的向后向执行计划。

对于更小计算的操作图，优化是至关重要的操作。因为这些计算很小，有很多子图模式可以匹配。因为不可变，生成的计算不能被枚举，一个核需要重新编译，与大操作库中预先编译的内核的固定数量相对应。
...
我们可以看到，在符号化程序中对于立即更新参数和操作剃度计算时有帮助的。

符号化库已经融合了程序因为python自身就时命令式的。例如，接下来的命令化程序融合了符号化的方法。
...
这个符号化图可以被编译进一个函数中被命令式执行。

## Small and Big Operations

使用大计算来修改已经存在的结构，用小计算来构建新部分。
